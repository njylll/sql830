...
<div class="page-container">
    <form class="form form-horizontal" id="formEdit" action="<%=path%>/score/scoreEditSubmit" method="post">
        <input type="hidden" class="input-text" value="${score.id}" placeholder="" id="id" name="id">
			...
			<span class="select-box" style="width: 400px;">
			  <select style="width: 390px;" class="select" size="1" name="no">
				   <c:forEach items="${userList}" var="c" varStatus="status">
                       <c:choose>
                           <c:when  test="${c.username == score.no}">
                               <option value="${c.id}" selected>学号:${c.username}--姓名:${c.realname}</option>
                           </c:when>
                           <c:otherwise>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" style="width: 400px;" class="input-text" value="${score.score}" placeholder="" id="score" name="score">
            </div>
        </div>
